        ...
        <div class="sub_backgroundColor data-detail-breadcrumb" style="width: 1110px;"
             :style='{"padding":"0 10px","boxShadow":"0 0 0px rgba(255,0,0,.3)","margin":"20px auto","borderColor":"rgba(135, 206, 250, 1)","borderRadius":"4px","borderWidth":"0","borderStyle":"solid","height":"54px"}'>
                ...
                <a v-if='storeupFlag' class="main_backgroundColor"
                   ...
                   @click="addNongyekejiCollection()" href="javascript:void(0)">
            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="blog-details-wrapper">
                        <div class="thumb">
                            <img :src="baseUrl+detail.nongyekejiPhoto" alt="image">
                        </div>
                        <div class="blog-details-header">
                            <h3 class="blog-details-title">{{title}}</h3>
                            <ul class="post-meta">
                                <li v-if="detail.nongyekejiUuidNumber">农业科技编号：
                                    {{detail.nongyekejiUuidNumber}}
                                </li>
                                <li v-if="detail.nongyekejiTypes">农业科技类型：
                                    {{detail.nongyekejiValue}}
                                </li>
                                <li v-if="detail.yanfaTime">科技研发时间：
                                    {{detail.yanfaTime}}
                                </li>
                                <li v-if="detail.nongyekejiClicknum">农业科技热度：
                                    {{detail.nongyekejiClicknum}}
                                </li>
                                <li v-if="detail.nongyekejiGongxiao">农业科技功效：
                                    {{detail.nongyekejiGongxiao}}
                                </li>
                            </ul>
                        ...
                        <!-- <div style="padding: 20px;border: 0px solid #f2f2f2;margin-top: 20px;display: flex">
                                 <div class="num-picker">
                                     <button @click="nongyekejiYuyue()" style="height:auto;" :style='{"padding":"0 10px","boxShadow":"0 0 0px rgba(255,0,0,.3)","margin":"0 5px","borderColor":"rgba(0,0,0,.3)","backgroundColor":"rgba(23, 124, 176, 1)","color":"rgba(255, 255, 255, 1)","borderRadius":"6px","borderWidth":"0","width":"auto","lineHeight":"40px","fontSize":"16px","borderStyle":"solid"}' type="button" class="layui-btn btn-submit">
                                         立即预约
                                     </button>
                                 </div>
                             </div>-->
                        </div>
                        <div class="blog-details-content" v-html="myFilters(detail.nongyekejiContent)">
                        </div>
                    </div>
                    <div class="comments-area" v-if="nongyekejiLiuyanDataList.length > 0">
                        <h3 class="title">
                            留言
                        </h3>
                        <ul class="comments-list">
                            <li class="single-comment" v-for="(item,index) in nongyekejiLiuyanDataList" v-bind:key="index">
                                <div class="thumb">
                        ...
                        <form class="comment-form">
                            <div class="row">
                                <div class="col-lg-12 form-group">
                                    <textarea name="comment-message" id="nongyekejiLiuyanText"
                                              placeholder="留言内容"></textarea>
                                </div>
                                <div class="col-lg-12">
                                    <button lay-submit lay-filter="nongyekejiLiuyanText"
                ...
                <div class="col-lg-4" v-if="nongyekejiRecommendList">
                    <div class="sidebar">
                        <div class="widget">
                            <h3 class="widget-title">系统推荐</h3>
                            <ul class="small-post-list">
                                <li class="small-post-single" v-for="(item,index) in nongyekejiRecommendList"
                                    v-bind:key="index"
                                    @click="jump('../nongyekeji/detail.jsp?id='+item.id)">
                                    <div class="thumb"><img width="65px" :src="baseUrl+item.nongyekejiPhoto"
                                                            alt="image">
                                    </div>
                                    <div class="content">
                                        <a class="main_color" class="date">{{item.nongyekejiName}}</a>
                                        <h6 v-if="item.nongyekejiNewMoney" class="post-title"><a
                                                style="color: red">{{item.nongyekejiNewMoney}}</a>RMB</h6>
                                    </div>
                                </li>
                            </ul>
                        </div>
    ...
    <script>
        ...
        var vue = new Vue({
            ...
            data: {
                ...
                //系统推荐
                nongyekejiRecommendList: [],
                    // 当前详情页表
                detailTable: 'nongyekeji',
                // 留言列表
                nongyekejiLiuyanDataList: [],
            },
            methods: {
                //分享
                share(){
                    let element = createElement("http://localhost:8080/zhinenghuazhongzhi/front/index.jsp?url="+"./pages/nongyekeji/detail.jsp?id="+this.id);
                    element.select();
                ...
                nongyekejiYuyue(){
                    let _this = this;
                    // localStorage.setItem('nongyekejiId', _this.detail.id);
                    // _this.jump("../nongyekejiOrder/add.jsp");

                    let data={
                        nongyekejiId:_this.detail.id,
                        yonghuId:localStorage.getItem("userid"),
                        nongyekejiOrderYesnoTypes:1,
                    }
                    // 提交数据
                    layui.http.requestJson('nongyekejiOrder/add', 'post', data, function (res) {
                        if(res.code == 0) {
                            layui.layer.msg('预约成功', {
                                time: 2000,
                                icon: 6
                            }, function () {
                                _this.jump("../nongyekejiOrder/list.jsp");
                            });
                        }else{
                ...
                // 收藏商品
                addNongyekejiCollection() {
                    let _this = this;
                    layui.http.request('nongyekejiCollection/list', 'get', {
                        page: 1,
                        limit: 1,
                        nongyekejiId: _this.detail.id,
                        ...
                        if (res.data.list.length == 1) {
                            layui.http.requestJson('nongyekejiCollection/delete', 'post', [res.data.list[0].id], function (res) {
                        ...
                        layui.http.requestJson('nongyekejiCollection/add', 'post', {
                            yonghuId: localStorage.getItem('userid'),
                            nongyekejiId: _this.detail.id,
                            nongyekejiCollectionTypes: 1,
                        }, function (res) {
        ...
        layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery', 'laypage', 'util'], function () {
            ...
            localStorage.setItem("goUtl","./pages/nongyekeji/detail.jsp?id="+ http.getParam('id'))
            ...
            http.request(`${vue.detailTable}/detail/` + id, 'get', {}, function (res) {
                // 详情信息
                vue.detail = res.data;
                vue.title = vue.detail.nongyekejiName;
                vue.detail.nongyekejiContent = vue.detail.nongyekejiContent.replaceAll("src=\"upload/","src=\""+vue.baseUrl+"upload/");
                // 轮播图片
                vue.swiperList = vue.detail.nongyekejiPhoto ? vue.detail.nongyekejiPhoto.split(",") : [];
                // 轮播图
                vue.$nextTick(() => {
                    carousel.render({
                        elem: '#swiper',
            ...
            http.request(`nongyekeji/gexingtuijian`, 'get', {
                page: 1,
                limit: 5,
                nongyekejiTypes: vue.detail.nongyekejiTypes,
                nongyekejiDelete: 1,
                shangxiaTypes: 1,//上架的
            }, function (res) {
                vue.nongyekejiRecommendList = res.data.list;
            });

            if (localStorage.getItem('userid')) {
                http.request('nongyekejiCollection/list', 'get', {
                    page: 1,
                    limit: 1,
                    nongyekejiId: vue.detail.id,
                    yonghuId: localStorage.getItem('userid'),
                }, function (res) {
                    if(res.data.total >0){
                        res.data.list.forEach(element => {
                            if (element.nongyekejiCollectionTypes == 1) {
                                vue.storeupFlag = 1;
                            }
                            if (element.nongyekejiCollectionTypes == 2) {
                                vue.zanFlag = 1;
                            }
                            if (element.nongyekejiCollectionTypes == 3) {
                                vue.caiFlag = 1;
                            }
                        });
            ...
            http.request(`${vue.detailTable}Liuyan/list`, 'get', {
                ...
                vue.nongyekejiLiuyanDataList = res.data.list;
                // 分页
                laypage.render({
                    elem: 'nongyekejiLiuyanPager',
            ...
            // 提交留言
            form.on('submit(nongyekejiLiuyanText)', function (data) {
                data.yonghuId = localStorage.getItem('userid');
                data.tableName = localStorage.getItem('userTable');
                data.nongyekejiLiuyanText =jquery("#nongyekejiLiuyanText").val();
                if(data.nongyekejiLiuyanText == null || data.nongyekejiLiuyanText == ''){
                    layer.msg("留言不能为空", {
                        time: 2000,
                        icon: 2
                    });
                ...
                data.nongyekejiId = vue.detail.id;
