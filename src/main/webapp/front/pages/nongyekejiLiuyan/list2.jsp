...
<div class="index-sec1">
    <div class="sec1-left fl">
        ...
        <p class="p2">农业科技留言<b></b></p>
        <ul>
            <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../nongyekejiLiuyan/list.jsp'?'on':''" @click="jump(item.url)">
        ...
        <div class="right-container sub_borderColor" style="overflow: auto" :style='{"padding":"20px","boxShadow":"0px rgba(255,0,0,.8)","margin":"0","backgroundColor":"#fff","borderRadius":"0","borderWidth":"1px","borderStyle":"solid","width":"80%"}'>
            <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;justify-content: space-between">
                <button @click="jump('../nongyekejiLiuyan/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加农业科技留言信息
                </button>
            </div>
        ...
        <!-- 级联表 -->
                        ...
                        <td>
                            <img v-if="item.nongyekejiPhoto" :src="baseUrl+item.nongyekejiPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            <span v-else >暂无照片</span>
                        </td>
                        <td><button v-if="item.nongyekejiLiuyanText!=null&&item.nongyekejiLiuyanText!=''" @click="showContent(item.nongyekejiLiuyanText)" type="button" class="layui-btn layui-btn-success">点击查看</button></td>
                        ...
                        <!--
                            <button @click="jump('../nongyekejiLiuyan/add.jsp?nongyekejiLiuyanId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                <i class="layui-icon">&#xe642;</i> 修改
                            </button>
                            <button @click="nongyekejiLiuyanModal = true" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
...
<script type="text/javascript">
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            deleteData(data){
                ...
                // 获取商品详情信息
                layui.http.requestJson(`nongyekejiLiuyan/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/nongyekejiLiuyan/list2.jsp")
        ...
        // var id = http.getParam('id');
            ...
            function pageList() {
                // 获取列表数据
                http.request('nongyekejiLiuyan/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('nongyekejiLiuyan/list', 'get', vue.searchForm, function (res1) {
