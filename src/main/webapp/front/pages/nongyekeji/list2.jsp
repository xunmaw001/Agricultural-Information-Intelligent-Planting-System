...
<div class="index-sec1">
    <div class="sec1-left fl">
        ...
        <p class="p2">农业科技<b></b></p>
        <ul>
            <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../nongyekeji/list.jsp'?'on':''" @click="jump(item.url)">
            ...
            <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;justify-content: space-between">
                <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;">
                    <div class="thisTableType-search" style="margin-left: 15px;"
                         :style='searchForm.nongyekejiTypes=="" || searchForm.nongyekejiTypes==null?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        全部
                    </div>
                    <div style="margin-left: 15px;" v-for="item in nongyekejiTypesList" :index="item.codeIndex" class="thisTableType-search"
                         :style='searchForm.nongyekejiTypes==item.codeIndex?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        {{item.indexName}}
                    </div>
                </div>
                <button @click="jump('../nongyekeji/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加农业科技信息
                </button>
            </div>
            <table class="layui-table" lay-skin="nob">
                ...
                <tbody>
                    <tr v-for="(item,index) in dataList" v-bind:key="index">
                        <td>{{item.nongyekejiName}}</td>
                        <td>{{item.nongyekejiUuidNumber}}</td>
                        <td><img :src="baseUrl+item.nongyekejiPhoto" style="width: 100px;height: 100px;object-fit: cover;"></td>
                        <td>{{item.nongyekejiValue}}</td>
                        <td>{{item.yanfaTime}}</td>
                        <td>{{item.nongyekejiClicknum}}</td>
                        <td>{{item.nongyekejiGongxiao}}</td>
                        <td><button v-if="item.nongyekejiContent!=null&&item.nongyekejiContent!=''" @click="showContent(item.nongyekejiContent)" type="button" class="layui-btn layui-btn-success">点击查看</button></td>
                        <td>{{item.shangxiaValue}}</td>
                        <td>{{item.insertTime}}</td>
                        <td style="width: 100px;">
                        <!--
                            <button @click="jump('../nongyekeji/add.jsp?nongyekejiId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                <i class="layui-icon">&#xe642;</i> 修改
                            </button>
                            <button @click="nongyekejiModal = true" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
...
<script type="text/javascript">
    ...
    var vue = new Vue({
        ...
        data: {
            ...
            searchForm: {
                page: 1
                ,limit: 8
                ,sort: 'nongyekeji_clicknum'
                ,order: "desc"//asc desc
                ,nongyekejiDelete: 1
                ,shangxiaTypes: 1
                        ,nongyekejiName: ""
                ,nongyekejiUuidNumber: ""
                ,nongyekejiPhoto: ""
                ,nongyekejiTypes: ""
                ,nongyekejiGongxiao: ""
                ,nongyekejiContent: ""
        ...
        methods: {
            ...
            deleteData(data){
                ...
                // 获取商品详情信息
                layui.http.requestJson(`nongyekeji/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/nongyekeji/list2.jsp")

        // var id = http.getParam('id');

        //农业科技类型的动态搜素
        $(document).on("click", ".thisTableType-search", function (e) {
            vue.searchForm.nongyekejiTypes = $(this).attr('index');
            pageList();
        });


            //当前表的 农业科技类型 字段 字典表查询
            nongyekejiTypesSelect();
           //当前表的 农业科技类型 字段 字典表查询方法
           ...
           function shangxiaTypesSelect() {
            ...
            function pageList() {
                // 获取列表数据
                http.request('nongyekeji/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('nongyekeji/list', 'get', vue.searchForm, function (res1) {
