...
<body style="background: #fff; ">
    ...
    <!-- 当前表的 -->
                ...
                <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
                    ...
                    <div class="layui-input-block">
                        <select name="chuqinyangzhiCollectionTypes" id="chuqinyangzhiCollectionTypes" lay-filter="chuqinyangzhiCollectionTypes">
                            <option value="">请选择</option>
                            <option v-for="(item,index) in chuqinyangzhiCollectionTypesList" v-bind:key="index" :value="item.codeIndex" :selected="detail.chuqinyangzhiCollectionTypes == item.codeIndex?true:false" :key="item.codeIndex">{{ item.indexName }}</option>
                        </select>
                    </div>
                </div>
                <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
    ...
    <script>
        ...
        var vue = new Vue({
            ...
            data: {
                ...
                ro:{
                    ...
                    yonghuId: false,
                    chuqinyangzhiCollectionTypes: false,
                    insertTime: false,
                    createTime: false,
                },
                detail: {
                    chuqinyangzhiId: '',
                    yonghuId: '',
                    chuqinyangzhiCollectionTypes: '',//数字
                    chuqinyangzhiCollectionValue: '',//数字对应的值
                    insertTime: '',
                    createTime: '',
                },
        ...
        layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload', 'laydate','tinymce'], function() {
           ...
           // 上传文件
            ...
            //修改
            if(http.getParam('chuqinyangzhiCollectionId')){
                http.request(`chuqinyangzhiCollection/info/${http.getParam('chuqinyangzhiCollectionId')}`, 'get', {}, function(data) {
            ...
            form.on('submit(thisSubmit)', function (data) {
                ...
                if(http.getParam('chuqinyangzhiCollectionId')){
                    data["id"]= http.getParam('chuqinyangzhiCollectionId')
                }
                // 提交数据
                http.requestJson(`chuqinyangzhiCollection/${http.getParam("chuqinyangzhiCollectionId")?"update":"add"}`, 'post', data, function (res) {
                    if(res.code == 0){
                        layer.msg('提交成功', {
                            time: 2000,
                            icon: 6
                        }, function () {
                            vue.jump("../chuqinyangzhiCollection/list.jsp");
                                http.remove('chuqinyangzhiCollectionId')
                            // back();
                        });
                    }else{
                        layer.msg(res.msg, {
       ...
       function chuqinyangzhiCollectionTypesSelect() {
           //填充下拉框选项
           layui.http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chuqinyangzhi_collection_types", "GET", {}, (res) => {
               if(res.code == 0){
