            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="blog-details-wrapper">
                        <div class="thumb">
                            <img :src="baseUrl+detail.nongyekejiCollectionPhoto" alt="image">
                        </div>
                        <div class="blog-details-header">
                            <h3 class="blog-details-title">{{title}}</h3>
                            <ul class="post-meta">
                                <li v-if="detail.nongyekejiCollectionTypes">类型：
                                    {{detail.nongyekejiCollectionValue}}
                                </li>
                            </ul>
                        ...
                        <!-- <div style="padding: 20px;border: 0px solid #f2f2f2;margin-top: 20px;display: flex">
                                 <div class="num-picker">
                                     <button @click="nongyekejiCollectionYuyue()" style="height:auto;" :style='{"padding":"0 10px","boxShadow":"0 0 0px rgba(255,0,0,.3)","margin":"0 5px","borderColor":"rgba(0,0,0,.3)","backgroundColor":"rgba(23, 124, 176, 1)","color":"rgba(255, 255, 255, 1)","borderRadius":"6px","borderWidth":"0","width":"auto","lineHeight":"40px","fontSize":"16px","borderStyle":"solid"}' type="button" class="layui-btn btn-submit">
                                         立即预约
                                     </button>
                                 </div>
                             </div>-->
                ...
                <div class="col-lg-4" v-if="nongyekejiCollectionRecommendList">
                    <div class="sidebar">
                        <div class="widget">
                            <h3 class="widget-title">系统推荐</h3>
                            <ul class="small-post-list">
                                <li class="small-post-single" v-for="(item,index) in nongyekejiCollectionRecommendList"
                                    v-bind:key="index"
                                    @click="jump('../nongyekejiCollection/detail.jsp?id='+item.id)">
                                    <div class="thumb"><img width="65px" :src="baseUrl+item.nongyekejiCollectionPhoto"
                                                            alt="image">
                                    </div>
                                    <div class="content">
                                        <a class="main_color" class="date">{{item.nongyekejiCollectionName}}</a>
                                        <h6 v-if="item.nongyekejiCollectionNewMoney" class="post-title"><a
                                                style="color: red">{{item.nongyekejiCollectionNewMoney}}</a>RMB</h6>
                                    </div>
                                </li>
                            </ul>
                        </div>
    ...
    <script>
        ...
        var vue = new Vue({
            ...
            data: {
                ...
                //系统推荐
                nongyekejiCollectionRecommendList: [],
                    // 当前详情页表
                detailTable: 'nongyekejiCollection',
            },
            methods: {
                //分享
                share(){
                    let element = createElement("http://localhost:8080/zhinenghuazhongzhi/front/index.jsp?url="+"./pages/nongyekejiCollection/detail.jsp?id="+this.id);
                    element.select();
                ...
                //预约
                nongyekejiCollectionYuyue(){
                    let _this = this;
                    // localStorage.setItem('nongyekejiCollectionId', _this.detail.id);
                    // _this.jump("../nongyekejiCollectionOrder/add.jsp");

                    let data={
                        nongyekejiCollectionId:_this.detail.id,
                        yonghuId:localStorage.getItem("userid"),
                        nongyekejiCollectionOrderYesnoTypes:1,
                    }
                    // 提交数据
                    layui.http.requestJson('nongyekejiCollectionOrder/add', 'post', data, function (res) {
                        if(res.code == 0) {
                            layui.layer.msg('预约成功', {
                                time: 2000,
                                icon: 6
                            }, function () {
                                _this.jump("../nongyekejiCollectionOrder/list.jsp");
                            });
                        }else{
        ...
        layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery', 'laypage', 'util'], function () {
            ...
            localStorage.setItem("goUtl","./pages/nongyekejiCollection/detail.jsp?id="+ http.getParam('id'))
            ...
            http.request(`${vue.detailTable}/detail/` + id, 'get', {}, function (res) {
                // 详情信息
                vue.detail = res.data;
                vue.title = vue.detail.nongyekejiCollectionName;
                // 轮播图片
                vue.swiperList = vue.detail.nongyekejiCollectionPhoto ? vue.detail.nongyekejiCollectionPhoto.split(",") : [];
                // 轮播图
                vue.$nextTick(() => {
                    carousel.render({
                        elem: '#swiper',
            ...
            http.request(`nongyekejiCollection/list`, 'get', {
                page: 1,
                limit: 5,
                nongyekejiCollectionTypes: vue.detail.nongyekejiCollectionTypes,
            }, function (res) {
                vue.nongyekejiCollectionRecommendList = res.data.list;
            });
