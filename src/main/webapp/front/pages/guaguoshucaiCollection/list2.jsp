...
<div class="index-sec1">
    <div class="sec1-left fl">
        ...
        <p class="p2">瓜果蔬菜收藏<b></b></p>
        <ul>
            <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../guaguoshucaiCollection/list.jsp'?'on':''" @click="jump(item.url)">
            ...
            <div style="display: flex;flex-wrap: wrap;justify-content: space-evenly;">
                <div v-for="(item,index) in dataList" :key="index" style="flex: 0 0 25%;padding: 3px">
                    <div :style='{"padding":"10px","borderColor":"var(--publicSubColor)","boxShadow":"0 0 6px var(--publicSubColor)","margin":"0 0 10px 0","borderColor":"rgba(0,0,0,.3)","backgroundColor":"#fff","borderRadius":"4px","borderWidth":"0","borderStyle":"solid"}'>
                            <i @click="deleteGuaguoshucaiCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: -10px;font-size:20px">&#xe640;</i>
                    <!-- class="animation-box"-->
                        <img @click="jumpCheck('../guaguoshucai/detail.jsp?id='+item.guaguoshucaiId , item.guaguoshucaiDelete == null?'':item.guaguoshucaiDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)"
                             ...
                             :src="item.guaguoshucaiPhoto?baseUrl+item.guaguoshucaiPhoto.split(',')[0]:''">
                        <div @click="jumpCheck('../guaguoshucai/detail.jsp?id='+item.guaguoshucaiId , item.guaguoshucaiDelete == null?'':item.guaguoshucaiDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                            <div v-if="item.guaguoshucaiNewMoney"
...
<script type="text/javascript">
    ...
    var vue = new Vue({
        ...
        data: {
            ...
            searchForm: {
                ...
                ,guaguoshucaiName: null
                ,guaguoshucaiTypes: null
            },
        ...
        methods: {
            ...
            deleteData(data){
                ...
                // 获取商品详情信息
                layui.http.requestJson(`guaguoshucaiCollection/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
            ...
            deleteGuaguoshucaiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('guaguoshucaiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/guaguoshucaiCollection/list2.jsp")

        // var id = http.getParam('id');

        //类型的动态搜素
        $(document).on("click", ".thisTableType-search", function (e) {
            vue.searchForm.guaguoshucaiCollectionTypes = $(this).attr('index');
            pageList();
        });
           ...
           function guaguoshucaiCollectionTypesSelect() {
               http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=guaguoshucai_collection_types", 'get', {}, function (res) {
                   if(res.code == 0){
            ...
            //级联表的 瓜果蔬菜类型 字段 字典表查询
            guaguoshucaiTypesSelect();
            //级联表的 瓜果蔬菜类型 字段 字典表查询方法
            ...
            function pageList() {
                // 获取列表数据
                http.request('guaguoshucaiCollection/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('guaguoshucaiCollection/list', 'get', vue.searchForm, function (res1) {
